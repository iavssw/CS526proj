...
int main(int argc, char **argv) {
    ...
    int streamingSetting = atoi(argv[1]);
    int baseAddress = atoi(argv[2]);
    int inputChannels = atoi(argv[3]);
    int height = atoi(argv[4]);
    int width = atoi(argv[5]);
    int outputChannels = atoi(argv[6]);
...
// printf("Read %f\n",
//        convWeight[outputChannels * inputChannels * FILTER_SIZE * FILTER_SIZE - 1]);
// printf("Read %f\n", convWeight[(11 * inputChannels * FILTER_SIZE * FILTER_SIZE) + (5 * FILTER_SIZE * FILTER_SIZE) +
//                                (4 * FILTER_SIZE) + (3)]);

// dataType test[10] = {0};
// test[0] = 42.42;

// writeArrayToMemory(memoryFileName, 0 + OUTPUTOFFSET, test, 10);

// float valueRead = readFloatFromMemory(memoryFileName, 45564);
// printf("Read %f from address %d in %s\n", valueRead, 45564, memoryFileName);

// readFromMemory(inputImage, convWeight, convBias);
// readFromStream(inputImage, convWeight, convBias);

// convolution(inputImage, convWeight, convBias, convOutput);

// writeToMemory(convOutput);
// writeToStream(convOutput);
