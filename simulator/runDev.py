...
if __name__ == "__main__":
    ...
    # if False:
        ...
        nextAddress = memManage.writeFloatArrayToMemory(mainMemoryFile, image, conv1.addr_image, conv1.num_input_pixels)
        
        # Conv1
        # Write the kernels to memory starting after max image size
        nextAddress = memManage.writeFloatArrayToMemory(mainMemoryFile, conv1_weights, conv1.addr_kernel, conv1.num_kernel_elements)
        # Write the bias to memory starting after max kernel size
        nextAddress = memManage.writeFloatArrayToMemory(mainMemoryFile, conv1_bias, conv1.addr_bias, conv1.num_bias_elements)
        
        # Conv3
        # Write the kernels to memory starting after max image size
        nextAddress = memManage.writeFloatArrayToMemory(mainMemoryFile, conv3_weights, conv3.addr_kernel, conv3.num_kernel_elements)
        # Write the bias to memory starting after max kernel size
        nextAddress = memManage.writeFloatArrayToMemory(mainMemoryFile, conv3_bias, conv3.addr_bias, conv3.num_bias_elements)
        
        # Conv5
        # Write the kernels to memory starting after max image size
        nextAddress = memManage.writeFloatArrayToMemory(mainMemoryFile, conv5_weights, conv5.addr_kernel, conv5.num_kernel_elements)
        # Write the bias to memory starting after max kernel size
        nextAddress = memManage.writeFloatArrayToMemory(mainMemoryFile, conv5_bias, conv5.addr_bias, conv5.num_bias_elements)
        
        # FC6
        # Write the kernels to memory starting after max image size
        nextAddress = memManage.writeFloatArrayToMemory(mainMemoryFile, fc6_weights, fc6.addr_kernel, fc6.num_kernel_elements)
        # Write the bias to memory starting after max kernel size
        nextAddress = memManage.writeFloatArrayToMemory(mainMemoryFile, fc6_bias, fc6.addr_bias, fc6.num_bias_elements)
    ...
    print(fc6.addr_output, fc6.num_output_pixels)
...
# Executing command: src/convR8_32_5 memory/mainmemory memory/stream6 memory/stream7 11 -1 141904 238144 -1 16 5 5 60
